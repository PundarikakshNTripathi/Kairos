...
export function Guide() {
  ...
  return (
    <div className="w-full max-w-6xl mx-auto flex flex-col md:flex-row gap-8 animate-in fade-in zoom-in-95 duration-500 pb-16 items-start">
      
      {/* Sidebar Navigation */}
      <div className="w-full md:w-64 shrink-0 md:sticky md:top-8 bg-card border border-border/50 p-4 rounded-xl shadow-xl flex flex-col gap-2">
        <h4 className="font-mono text-sm uppercase tracking-widest text-muted-foreground mb-2">Contents</h4>
        <button onClick={() => scrollTo('philosophy')} className="text-left text-sm hover:text-primary transition-colors py-1">The Core Philosophy</button>
        <button onClick={() => scrollTo('why-it-exists')} className="text-left text-sm hover:text-primary transition-colors py-1">Why It Exists</button>
        <button onClick={() => scrollTo('what-it-solves')} className="text-left text-sm hover:text-primary transition-colors py-1">What It Solves</button>
        <button onClick={() => scrollTo('hybrid-cloud-sync')} className="text-left text-sm hover:text-primary transition-colors py-1">Hybrid Cloud Sync</button>
        <button onClick={() => scrollTo('security-measures')} className="text-left text-sm hover:text-primary transition-colors py-1">Security Measures</button>
        <button onClick={() => scrollTo('how-to-use')} className="text-left text-sm hover:text-primary transition-colors py-1">How to Use It</button>
      </div>

      {/* Main Content */}
      <div className="flex-1 space-y-8">
        <div id="philosophy" className="space-y-4 pt-2">
        ...
        <div className="grid gap-6 md:grid-cols-2">
          <Card id="why-it-exists" className="border-border/50 bg-card shadow-xl scroll-mt-20">
            <CardHeader>
          ...
          <Card id="what-it-solves" className="border-border/50 bg-card shadow-xl scroll-mt-20">
            <CardHeader>
              <CardTitle className="text-xl font-light tracking-wide">What It Solves</CardTitle>
            </CardHeader>
            <CardContent className="text-muted-foreground leading-relaxed">
              It eliminates priority bloat. The Executive Focus board enforces a strict limit on your active priorities. The command palette journal offers instant, frictionless brain dumping.
            </CardContent>
          </Card>
        </div>

        <div className="grid gap-6 md:grid-cols-2">
          <Card id="hybrid-cloud-sync" className="border-border/50 bg-card shadow-xl scroll-mt-20">
            <CardHeader>
              <CardTitle className="text-xl font-light tracking-wide">Hybrid Cloud Sync</CardTitle>
            </CardHeader>
            <CardContent className="text-muted-foreground leading-relaxed">
              Kairos operates on a hybrid storage model. When you log a journal entry or update a priority, the data is instantly written to your browser's synchronous <code className="bg-muted px-1 rounded">localStorage</code> for zero-latency UI updates. In the background, it silently syncs this data to a remote Supabase PostgreSQL database, ensuring your progress is never lost across devices.
            </CardContent>
          </Card>
          
          <Card id="security-measures" className="border-border/50 bg-card shadow-xl scroll-mt-20">
            <CardHeader>
        ...
        <div id="how-to-use" className="space-y-4 pt-8 border-t border-border/30 scroll-mt-20">
